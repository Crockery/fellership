import { type PROFILE_KEYS, parseField } from "../shared";
import { SINGLE_VALUE_KEYS } from "./constants";

const getValueFromParts = (parts: string[], key: string) => {
	const part = parts.find((part) => part.startsWith(key));

	if (part) {
		const value = parseField(part)[1];
...
export const parseProfile = (original: string) => {
	let profileString = original;

	profileString = original.replace("HUDSettingsProfile=(", "");
	profileString = profileString.slice(0, -1);

	const parts = profileString.split(",");

	const parsed_single_value = SINGLE_VALUE_KEYS.reduce(
		(acc, item) => {
			acc[item] = getValueFromParts(parts, item);

			return acc;
		},
		{} as Record<PROFILE_KEYS, string | undefined>,
	);

	if (!parsed_single_value.ProfileName) {
		throw "Unable to parse profile.";
	}

	return parsed_single_value;
};
